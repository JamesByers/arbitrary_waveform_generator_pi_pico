...
while(toggle):
    ...
    for freq in (1e5,2e5,3e5,4e5,5e5,1e6,2e6):
        setupwave(wavbuf[ibuf],freq,wave1); ibuf=(ibuf+1)%2
    freq=2e5    
    ...
    for duty in (.8,0.5,0.4,0.3,0.2,0.1,0.05,0.02):
        wave1.pars[1]=duty
        setupwave(wavbuf[ibuf],freq,wave1); ibuf=(ibuf+1)%2

    #pulse: step through rise time,fall time
    wave1.amplitude=1.0
    for rise in (0.02,0.05,0.20):
        wave1.pars=[rise,0.1,0.0]
        setupwave(wavbuf[ibuf],freq,wave1); ibuf=(ibuf+1)%2
    for fall in (0.02,0.05,0.20):
...
if(1==0):
    ...
    for freq in (1e4,2e4,1e5,2e5,5e5,1e6,2e6,5e6,1e7,2e7):
        setupwave(wavbuf[ibuf],freq,wave1); ibuf=(ibuf+1)%2
    freq=2e5
